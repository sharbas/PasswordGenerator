...
import { getRandomChar, getSpecialChar } from '../utils/utils.js'
import { toast } from 'react-toastify'
// import { useDeletePasswordMutation, useSavePasswordMutation } from '../slices/usersApiSlice.js'
// import axios from 'axios';
import { usersApi } from '../axiosApi/axiosInstance.js'
...
const saveHandler = async () => {
  try {
    // Assuming userInfo.token contains the JWT token
    const headers = {
      Authorization: `Bearer ${userInfo.token}`,
      // Add other headers if needed
    };

    // console.log(headers, 'header in saveHandler');

    const response = await usersApi.post(`/save-password`, { result }, { headers });

    // console.log(response.data, 'res in savePassword function');
...
const deleteHandler = async (id) => {
  try {
    ...
    const response = await usersApi.delete(`/delete-saved`, { headers, data: {saved: id} });
